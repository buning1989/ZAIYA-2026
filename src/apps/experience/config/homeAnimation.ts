/* —— 体验模式首页动画注册表 ——
 *
 * 独立于演示模式的动画注册表。即使当前引用同一份动画文件，
 * 也通过独立注册表引用，确保未来替换体验模式动画时不会影响演示模式。
 *
 * 引用隔离原则：两个模式的注册表是独立的对象实例，
 * 修改一方不会影响另一方的引用。 */
import type { HomeTimePhase } from "@/lib/homeTimePhase";

/** 体验模式：各时间段对应的在在首页动画素材路径 */
export const EXPERIENCE_HOME_PHASE_SCENE: Record<HomeTimePhase, string> = {
...
/** 体验模式：各时间段对应的气泡文案 */
export const EXPERIENCE_HOME_PHASE_BUBBLE_TEXT: Record<HomeTimePhase, string> = {
